...
void glsl_error(struct glsl_parse_context *c, const char *s);
...
%defines "glsl-parser/glsl.parser.h"
...
section_statement 	: SECTION section_identifier LEFT_BRACE declaration_statement_list RIGHT_BRACE SEMICOLON
				{ $$ = new_glsl_node(context,SECTION_STATEMENT, $2, $4, NULL); }
			;

declaration_statement_list : declaration
				{ $$ = new_glsl_node(context, DECLARATION_STATEMENT_LIST, $1, NULL); }
			| declaration_statement_list declaration
				{ $$ = new_glsl_node(context, DECLARATION_STATEMENT_LIST, $1, $2, NULL); }
			;
...
section_identifier : IDENTIFIER { $$ = new_glsl_identifier(context, $1); }
			;
