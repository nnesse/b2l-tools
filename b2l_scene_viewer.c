...
static struct math3d_mat4 *object_local_transform(lua_State *L, void *blob, const char *object, const char *scene, int frame)
{
	lua_getfield(L, -1, "scenes");
	lua_getfield(L, -1, scene);
	lua_getfield(L, -1, "objects");
	lua_getfield(L, -1, object);
	lua_getfield(L, -1, "object_transform_array_offset");
	int offset = lua_tointeger(L, -1);
	lua_pop(L, 5);
	return (struct math3d_mat4 *)(((uint8_t *) blob) + offset);
}

static int object_parent(lua_State *L)
{
	lua_getfield(L, -1, "objects");
...
int main(int argc, char **argv)
{
	...
	if (!scene_name) {
		...
		while (lua_next(L, scenes_idx)) {
			scene_idx = lua_gettop(L);
	...
	while (glplatform_process_events()) {
		...
		math3d_mat4_identity(&view_pan);
		view_pan.v[3][0] = g_offset.v[0] + g_offset_next.v[0];
		view_pan.v[3][1] = g_offset.v[1] + g_offset_next.v[1];
		view_pan.v[3][2] = g_offset.v[2] + g_offset_next.v[2];
		math3d_mat4_mul(&view, &view_pan, &temp);
		view = temp;
		...
		while (lua_next(L, -2)) {
			...
			if (b2l_is_mesh_object(L, b2l_data_idx, object_name)) {
				lua_pushstring(L, object_name);
				lua_pushvalue(L, b2l_data_idx);
				struct math3d_mat4 *m;
				math3d_mat4_identity(&model);
				do {
					m = object_local_transform(L, blob, lua_tostring(L, -2), scene_name, frame);
					math3d_mat4_mul(&model, m, &temp);
					model = temp;
					object_parent(L);
				} while(!lua_isnil(L, -2));
				lua_pop(L, 2);
